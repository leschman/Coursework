...
document.write("<script language=\"JavaScript\" src=\"search/zoom_pageinfo.js\" charset=\"" + Charset + "\"><\/script>");
...
function ZoomSearch() 
{
    ...
    while (arrayline < matches && arrayline < result_limit) {
        ...
        if (DisplayNumber == 1)
         ...
         // EDITED FOR COD. Check for "printed" in the url and do not display a link if it is found. (Only display title)
        if(pgurl.search('printed/COD_')!=-1)
        {
        DisplayURL = 0;
			if (DisplayTitle == 1)
			{
				// Fix title to show a nice title, rather than the file name
				pgtitleSplit = pgtitle.split('_');
				if(pgtitleSplit[2] > 0)
				{
					pgtitle = 'Printed text, Appendix '+ pgtitleSplit[1] + ', page ' + pgtitleSplit[1]+ '-' + pgtitleSplit[2];
				}
				else
				{
					pgtitle = 'Printed text, page '+ pgtitleSplit[1];
				}
				PrintHighlightDescription(pgtitle);
			}
			else
				document.writeln(pgurl);
        }
        else
        {
        DisplayURL = 1;
