...
init:	sw	$s1, ($sp)	# write ones to the stackpointer's address
	...
	sub	$sp, $sp, 8	# subtract 8 bytes from stackpointer (push)
	ble	$t0, $t9, init	# take loop while $t0 <= $t9

	li	$t0, 1		# reset counter variable to 1

outer:	add 	$t0, $t0, 2	# increment counter variable  by 2, (start at 3, only check odd numbers)
	mul	$t1, $t0, $t0	# multiply $t0 by $t0, all numbers left before this prime. 
